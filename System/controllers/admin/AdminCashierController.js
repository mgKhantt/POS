...
const postCheckOut = async (req, res) => {
    ...
    try {
        // ✅ Step 1: Update stock safely with transaction
        ...
        // ✅ Step 2: Save order record in the same transaction
        ...
        // ✅ Step 3: Commit transaction (only now DB changes are final)
        ...
        res.setHeader(
            "Content-Disposition",
            `attachment; filename=voucher-${Date.now()}.pdf`
        );
        ...
        cart.forEach((item) => {
            ...
            doc.text(`x${item.qty}`, xQty - 10, doc.y - 10, {
                width: 30,
                align: "center",
            });

            // Total (right aligned - crucial)
            doc.text(`$${itemTotal}`, xTotal - 10, doc.y - 10, {
                width: 50,
                align: "right",
            });
        ...
        const drawTotalLine = (label, amount, font, size, amountWidth = 60) => {
            ...
            doc.text(label, MARGIN, yPos, {
                width: amountX - MARGIN,
                align: "left",
            });

            // Amount on the right, fixed width, right-aligned
            doc.text(`$${amount.toFixed(2)}`, amountX, yPos, {
                width: amountWidth,
                align: "right",
            });
